...
Set `--title "Office workstation"` to identify the server in the browser tab, PIN page, and
control header. When the option is omitted, the title is `GUI Remote Controll`. The server
escapes the value as text and limits it to 200 characters.
...
The separate permission indicator is driven by WebSocket messages from the server. It has three
authoritative states:

| Indicator text | Meaning |
| --- | --- |
| **Control available** | The client may send mouse, wheel, keyboard, text, and IME operations. |
| **Server is operating** | Physical input was detected on the server; remote input is temporarily suspended. |
| **Control access restricted** | The server is view-only, physical-input monitoring is unavailable, or the WebSocket is disconnected. |

The browser stops producing remote input as soon as permission is lost. The server also validates
every input message, so a modified or delayed client cannot bypass the indicator state.
...
the same current control-permission label as the header indicator.
...
## Physical server input priority

The server monitors physical mouse movement, clicks, wheel activity, and keyboard input. Each
physical event grants the server user an exclusive 0.8-second input lease. Continued activity
renews that deadline; after 0.8 seconds without physical input, remote control becomes available
again automatically.

Remote events generated by this application are marked as injected by `pynput` and do not renew
the lease. When physical input takes priority, the server releases remotely held keys and mouse
buttons before rejecting further remote input. The lease is shared across all browser clients,
so no client can continue controlling while the person at the server is active.

If global input monitoring cannot start because of an operating-system permission or session
problem, the server deliberately reports **Control access restricted**. Screen viewing,
monitor selection, pings, and clipboard operations remain available.
...
## Server input method control

The **IME: On** / **IME: Off** button changes the input mode on the server computer. This is an
...
Start with `--view-only` to stream frames while ignoring pointer, wheel, keyboard, text, and IME
...
the same operating-system desktop, so multiple controlling clients can affect one another.
Physical server input takes priority over every client, but remote clients do not arbitrate among
themselves. Use view-only mode or a low client limit when exclusive remote control is required.
...
- `GET /api/status` returns version, title, platform, control state, clipboard availability, and client counts.
...
- IME control depends on the foreground application and installed platform input method framework.
